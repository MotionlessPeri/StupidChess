...
3. `wire_local_match_widget_graph.py`
   ...
   - 用法：
     ...
     - `python tools\wire_local_match_widget_graph.py --wire-construct`（Preserve 模式下会跳过 Construct 重建并给出警告）
     - `python tools\wire_local_match_widget_graph.py --clear --wire-construct`（完整清图重建，包含 Construct）
