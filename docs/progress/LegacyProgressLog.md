...
79. Prototype A 表面职业映射回归修复（第 78 条二次修复）：
    - 发现 `Show Red View` 下红方棋子表面职业仍可能退化为实际职业（`Bootstrap Scrambled` 下最明显），说明“按标准摆法列表扫描推导表面职业”在当前原型口径下存在红方映射偏差。
    - 将表面职业推导改为“按初始合法位模板坐标直算”（红方用 canonical 模板、黑方按 `Y` 镜像），不再依赖运行时 `SetupStandardPlacements*` 扫描结果。
    - 该修复按“第 78 条回归/二次修复”记录，避免重复将同类问题当作新问题处理。
